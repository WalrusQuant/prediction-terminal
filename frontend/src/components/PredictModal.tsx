...
interface PredictModalProps {
  model: Model;
  onSinglePredict: (modelId: string, inputData: Record<string, number>, label?: string) => Promise<{ prediction: { predicted_value: number } }>;
...
export function PredictModal({ model, onSinglePredict, onBatchPredict, onClose }: PredictModalProps) {
  ...
  const handleSinglePredict = async () => {
    ...
    setLoading(true);
    try {
      const result = await onSinglePredict(model.id, numericData, label || undefined);
      setPredictionResult(result.prediction.predicted_value);
    } catch (e) {
              ...
              {predictionResult !== null && (
                <div style={{
                  ...
                  <div style={{ fontSize: '24px', color: 'var(--cyan)', fontWeight: 'bold' }}>
                    {predictionResult.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                  </div>
                </div>
              )}
